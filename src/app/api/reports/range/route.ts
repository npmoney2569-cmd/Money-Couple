...
export async function GET(request: Request) {
  ...
  const start = url.searchParams.get("start") || new Date(new Date().getFullYear(), new Date().getMonth(), 1).toISOString().slice(0, 10);
  const end = url.searchParams.get("end") || new Date(new Date().getFullYear(), new Date().getMonth() + 1, 0).toISOString().slice(0, 10);
